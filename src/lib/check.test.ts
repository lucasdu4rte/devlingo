...
const multiLarge: Exercise = {
  type: "multi-choice",
  prompt: { en: "?" },
  options: Array.from({ length: 11 }, (_, i) => ({ en: `o${i}` })),
  correct: [2, 10],
};
...
describe("isCorrect", () => {
  ...
  test("multi-choice with double-digit indexes", () => {
    expect(isCorrect(multiLarge, [10, 2])).toBe(true);
    expect(isCorrect(multiLarge, [2, 10])).toBe(true);
    expect(isCorrect(multiLarge, [2])).toBe(false);
  });
